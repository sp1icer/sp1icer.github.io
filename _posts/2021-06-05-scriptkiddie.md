...
It appears that we've landed on a new hacker's site designed to run a few commands that we'd use as attackers normally - it has fields for running an `nmap` scan, generating a `msfvenom` payload, and searching for exploits via `searchsploit`. I realize that this might not be the most realistic scenario, but just use some imagination and think of it as a VPS you stumbled across 😉
